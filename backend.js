/**
 * Lorebook Web Scraper — backend module. v1.13
 *
  ...
  try {
    switch (payload.type) {
      ...
      case 'lws:condense': {
        ...
        const variants = [
          ['userId, body', () => spindle.generate(userId, body)],
          ['body, userId', () => spindle.generate(body, userId)],
          ['body.userId', () => spindle.generate({ ...body, userId })],
          ['body only', () => spindle.generate(body)],
          ['userId, prompt, opts', () => spindle.generate(userId, prompt, { connectionId, maxTokens })],
          ['prompt, opts', () => spindle.generate(prompt, { connectionId, maxTokens })],
        ];
        ...
        if (!condensed) {
          return fail(`generate produced no text. Tried: ${failures.slice(0, 6).join(' | ')}`);
        }
...
spindle.log.info('Lorebook Web Scraper backend ready (v1.13). Continue fix registered.');
