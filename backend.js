/**
 * Lorebook Web Scraper — backend module. v1.8
 *
  ...
  try {
    switch (payload.type) {
      ...
      case 'lws:list_connections': {
        const result = await attempt('connections.list', [
          ['options.userId', () => spindle.connections.list({ userId })],
          ['second argument', () => spindle.connections.list({}, userId)],
          ['no arguments', () => spindle.connections.list()],
        ]);
        ...
        return reply({
          ...
          connections: raw
            .map((c) => ({ id: c.id, name: c.name || c.label || c.model || '(unnamed)', model: c.model || '' }))
            .filter((c) => c.id),
        });
      }
...
spindle.log.info('Lorebook Web Scraper backend ready (v1.8). Continue fix registered.');
