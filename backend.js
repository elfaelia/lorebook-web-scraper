/**
 * Lorebook Web Scraper — backend module. v1.11
 *
  ...
  try {
    switch (payload.type) {
      ...
      case 'lws:list_connections': {
        ...
        let result;
        try {
          result = await attempt('connections.list', [
            ['options.userId', () => conn.list({ ...options, userId })],
            ['options only', () => conn.list(options)],
            ['userId only', () => conn.list({ userId })],
            ['second argument', () => conn.list(options, userId)],
            ['no arguments', () => conn.list()],
          ]);
        } catch (err) {
          return reply({ type: 'lws:connections', connections: [], note: `connections.list failed: ${message(err)}` });
        }
        ...
        return reply({
          ...
          connections,
          note: connections.length ? `Loaded ${connections.length}.` : `connections.list returned no usable rows. Result keys: ${result && typeof result === 'object' ? Object.keys(result).join(', ') : typeof result}`,
        });
      }
      ...
      case 'lws:condense': {
        ...
        const prompt = [
          `Rewrite the reference material below as a compact factual note about ${title}, for use as a`,
          `lorebook entry in a roleplay setting. Hard limit: roughly ${targetTokens} tokens`,
          `(about ${Math.round(targetTokens * 0.75)} words). Shorter is better than longer.`,
          '',
          'Rules:',
          '- Write flowing prose in a neutral encyclopedic register. No headings, no bullet points, no markdown.',
          '- Keep concrete specifics: named techniques, mechanisms, terminology, cause and effect.',
          '- Drop citations, dates of publication, article structure, cross-references, and "see also" material.',
          '- Drop anything about who studied it or where it was published. Keep only what the thing IS and how it works.',
          '- Do not address the reader. Do not mention the source, the article, or that this is a summary.',
          '- Output only the note itself, with no preamble, no title, and no closing remark.',
          '',
          'Reference material:',
          '"""',
          text.slice(0, 40000),
        ...
        const request = {
          ...
          maxTokens: Math.max(256, Math.ceil(targetTokens * 2)),
          temperature: 0.3,
        };
        if (connectionId) request.connectionId = connectionId;

        const result = await attempt('generate', [
          ['request object', () => spindle.generate(request)],
          ['request + userId', () => spindle.generate(request, userId)],
          ['prompt string', () => spindle.generate(prompt, { connectionId, maxTokens: request.maxTokens })],
        ]);
        ...
        if (!condensed) {
          return fail(`The condenser returned nothing usable. Raw response keys: ${result && typeof result === 'object' ? Object.keys(result).join(', ') : typeof result}`);
        }

        return reply({ type: 'lws:condensed', text: condensed, originalLength: text.length });
      }
...
spindle.log.info('Lorebook Web Scraper backend ready (v1.11). Continue fix registered.');
